---
title: "FlareOn 2014 - Challenge 4: PDF Stream Extraction & Shellcode Analysis"
date: 2026-07-07
category: FlareOn
description: Extracting compressed zlib streams from a hidden PDF exploit wrapper and statically analyzing dynamic shellcode resolutions.
tags:
  - reverse
  - flareon
  - ctf
...
# FlareOn 2014 - Challenge 4: PDF Stream Extraction & Shellcode Analysis

## 1. Initial Analysis of the PDF Structure

The challenge provides a PDF file named `APT9001.pdf`. Attempting to open the PDF in standard readers displays no visible content:

![Blank PDF Document](blank-pdf.png)
![PDF Reader Layout](pdf-reader-view.png)

Because the PDF displays nothing, we inspect the file structure in hex mode. We identify a compressed data segment following standard PDF stream header declarations:

![Zlib Stream Magic Bytes](zlib-magic-hex.png)

The PDF format uses `FlateDecode` filters to compress object stream data. The `stream` keyword occurs immediately before a carriage return/line feed (`0D 0A`), followed by the zlib compression header `78 DA`.

---

## 2. Automating Stream Extraction

We develop a Python script to scan the raw PDF file, match `stream...endstream` regex blocks, and decompress the zlib chunks:
...
def extract_and_decompress_pdf_streams(filename):
    try:
    ...
    output_dir = "pdf_streams"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        print(f"[*] Created output directory: {output_dir}\n")
    
    stream_pattern = re.compile(rb'stream\s*(.*?)\s*endstream', re.DOTALL)
    streams = stream_pattern.findall(pdf_data)
    
    if not streams:
        print("[WARNING] No streams found in PDF")
        return []
    ...
    for idx, stream_data in enumerate(streams, 1):
        try:
            stream_data = stream_data.strip()
            decompressed = zlib.decompress(stream_data)
            decompressed_streams.append(decompressed)
            
            output_file = os.path.join(output_dir, f"stream_{idx:02d}.bin")
            with open(output_file, "wb") as f:
                f.write(decompressed)
            
            print(f"[+] Stream {idx} - Decoded ({len(stream_data)} -> {len(decompressed)} bytes)")
            
        except zlib.error as e:
            print(f"[-] Stream {idx} - Decompression failed: {e}")
            output_file = os.path.join(output_dir, f"stream_{idx:02d}_raw.bin")
            with open(output_file, "wb") as f:
                f.write(stream_data)
...
if __name__ == "__main__":
    extract_and_decompress_pdf_streams("APT9001.pdf")
```

Running the script extracts three decompressed binary streams:

![Extraction Script Output](script-output.png)
![Dumped Binary Streams](streams-dumped.png)

---

## 3. Deobfuscating the JavaScript Payload

Stream 1 contains a long hex-encoded character sequence:

![Stream 1 Hex Data](stream1-hex.png)

Decoding these hex values reveals an obfuscated JavaScript block that performs a heap-spray attack:
...
## 4. Shellcode Analysis & Execution Attempts

The script extracts the shellcode buffer packed in the `IxTUQnOvHg` variable. We write a custom program to compile and run this shellcode dynamically. Using a shellcode-to-executable utility (`shellcode2exe`), we construct a test harness executable.

When executed, the program crashes, raising an `EXCEPTION_ACCESS_VIOLATION` error:

![Debugger Exception Window](crash-eax.png)
![Debugger Memory Map Crash](crash-dump.png)

The shellcode attempts to dereference an invalid memory location at runtime, indicating that it relies on context initialized within the PDF reader process (e.g., specific DLL hooks or document parameters). 

Rather than running the payload directly outside the PDF reader, we analyze the static properties of the shellcode. The routine dynamically resolves API addresses using the Process Environment Block (PEB) structure. It looks up `LoadLibraryA` and `GetProcAddress` to dynamically load `user32.dll` and calls `MessageBoxA` to output the flag.

We extract the final flag payload:
`big.sho.9001@flare-on.com`
